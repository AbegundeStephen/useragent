...
const locationTask = async ({data,error}) => {
    ...
    if (data) {
        ...
        console.log("Locations From task manager: " + JSON.stringify(locations[0].coords))
    ...
    //   console.log("deviceAddress: "+address)
...
const LocationView = () => {
    ...
    useEffect(() => {
        TaskManager.getRegisteredTasksAsync().then((tasks) => {
            if (tasks.find((f) => f.taskName === LOCATION_TASK_NAME)) {
                TaskManager.getTaskOptionsAsync(LOCATION_TASK_NAME).then((locationData) =>{
                    console.log("Inside registered task: "+ locationData)
                    setLocation(locationData)
                })
            }
        })
    },[])
    console.log("Location from state: "+ JSON.stringify(location))
  return (
